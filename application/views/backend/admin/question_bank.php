...
<div class="row mb-3">
  <div class="col-md-4 mb-2">
    <a class="qb-tab-card <?php echo $active_tab === 'create_exam' ? 'active' : ''; ?>" href="<?php echo site_url('admin/question_bank?qb_tab=create_exam'); ?>">
      <h5>1. Setup Exams &amp; Sections</h5>
      <small>Create exam names (SBI PO, SSC CGL, Class 10...) and their sections. Questions and mock tests are organized under these.</small>
    </a>
  </div>
  <div class="col-md-4 mb-2">
    <a class="qb-tab-card <?php echo $active_tab === 'upload' ? 'active' : ''; ?>" href="<?php echo site_url('admin/question_bank?qb_tab=upload'); ?>">
      <h5>2. Add Questions (Single &amp; Bulk)</h5>
      <small>Type questions one by one, or import hundreds at once from a CSV/Excel file using the template.</small>
    </a>
  </div>
  <div class="col-md-4 mb-2">
    <a class="qb-tab-card <?php echo $active_tab === 'search' ? 'active' : ''; ?>" href="<?php echo site_url('admin/question_bank?qb_tab=search'); ?>">
      <h5>3. Manage Questions</h5>
      <small>Search, filter, edit, duplicate, archive, and send questions for review approval.</small>
    </a>
  </div>
</div>
<div class="qb-help mb-3"><strong>How it works:</strong> Setup exams &amp; sections → Add questions → Manage &amp; approve them → then build and publish the test in <a href="<?php echo site_url('admin/content_public_exams'); ?>">Exam Pattern Builder</a>.</div>
...
<div class="card qb-panel"><div class="card-body">
  ...
  <?php
    $tax_exam_counts = $taxonomy_counts['exams'] ?? array();
    $tax_section_counts = $taxonomy_counts['sections'] ?? array();
  ?>
  <?php if(empty($exam_names)): ?><div class="alert alert-light border mb-0">No exam created yet. Start here: add your first exam name above, then its sections.</div><?php else: ?>
  <div class="row"><?php foreach($exam_names as $name): ?><div class="col-md-4 mb-3"><div class="qb-help h-100"><strong><?php echo html_escape($name); ?></strong> <span class="badge badge-info-lighten"><?php echo (int)($tax_exam_counts[$name] ?? 0); ?> questions</span><div class="mt-2"><?php $items=$sections_by_exam[$name]??array(); if(empty($items)): ?><span class="text-muted small">No category added yet.</span><?php else: foreach($items as $sec): ?><span class="qb-chip"><?php echo html_escape($sec); ?> · <?php echo (int)($tax_section_counts[$name][$sec] ?? 0); ?></span><?php endforeach; endif; ?></div></div></div><?php endforeach; ?></div>
  <?php endif; ?>
</div></div>
<?php endif; ?>
...
<div class="row">
  <div class="col-xl-5 mb-3">
    <div class="card qb-panel"><div class="card-body">
      <h4 class="header-title">Bulk Import (CSV / Excel)</h4>
      <p class="text-muted small">Use this for large question sets. Download the template, fill in your questions, then import the file.</p>
