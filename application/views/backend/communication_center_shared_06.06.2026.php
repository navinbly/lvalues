        ...
        <form id="communication-form" method="post" action="<?php echo $action; ?>" class="card">
            <div class="card-body">
                ...
                <div class="row">
                    <?php
                    $selects = [
                        'category_id' => ['Category', $options['categories'], 'name'],
                        'class_id' => ['Class', $options['classes'], 'name'],
                        'subject_id' => ['Subject', $options['subjects'], 'name'],
                        'course_id' => ['Course interest', $options['courses'], 'title'],
                        'batch_id' => ['Batch', $options['batches'], 'title'],
                    ];
                    if ($role === 'admin') {
                        $selects['tutor_id'] = ['Tutor', $options['tutors'], 'first_name'];
                    }
                    foreach ($selects as $name => $definition): ?>
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="<?php echo $name; ?>"><?php echo html_escape($definition[0]); ?></label>
                                <select class="form-control" id="<?php echo $name; ?>" name="<?php echo $name; ?>">
                                    <option value="">All</option>
                                    <?php foreach ($definition[1] as $item): ?>
                                        <?php $label = $definition[2] === 'first_name' ? trim(($item['first_name'] ?? '') . ' ' . ($item['last_name'] ?? '')) : ($item[$definition[2]] ?? ''); ?>
    ...
    <div class="card-body">
        ...
        <div class="table-responsive">
            <table class="table table-sm table-hover">
                <thead><tr><th>Date</th><th>Campaign</th><th>Student</th><th>Channel</th><th>Status</th><th>Delivery note</th></tr></thead>
                <tbody>
                <?php if (empty($delivery_logs)): ?><tr><td colspan="6" class="text-center text-muted">No delivery logs yet.</td></tr><?php endif; ?>
                <?php foreach ($delivery_logs as $log): ?><tr>
                    ...
                    <td><?php echo html_escape(trim(($log['first_name'] ?? '') . ' ' . ($log['last_name'] ?? ''))); ?></td>
